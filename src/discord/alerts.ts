    ...
    try {
      await channel.send({
        embeds: [new EmbedBuilder()
          ...
          .setDescription('The alert system is working correctly. This is only a test and does not indicate an outage.')
          .setFooter({ text: 'AniList API Monitor' })
          .setTimestamp()]
      });
      return true;
    } catch (error) {
      this.logger.error('Failed to send test alert.', { error: error instanceof Error ? error.message : String(error) });
      return false;
    }
  }

  async sendStatusMessage(result: HealthResult | null = this.db.snapshot.lastCheck): Promise<boolean> {
    const state = this.db.snapshot;
    const channel = await this.getChannel(state.config.statusChannelId);
    if (!channel) {
      this.logger.warn('Status message not sent because no valid status channel is configured.');
      return false;
    }
    ...
    const embed = new EmbedBuilder()
      ...
      .setFooter({ text: 'AniList API Monitor • New check' });

    if (status !== 'HEALTHY' && error !== 'None') embed.addFields({ name: 'Details', value: error });
    if (result?.retryAfterSeconds != null) embed.addFields({ name: 'Retry After', value: `**${result.retryAfterSeconds}s**`, inline: true });
    if (state.outageStartedAt) embed.addFields({ name: 'Current Outage', value: `**${formatDuration(Date.now() - state.outageStartedAt)}**`, inline: true });

    try {
      await channel.send({ embeds: [embed], allowedMentions: { parse: [] } });
      return true;
    } catch (error) {
      this.logger.error('Failed to send AniList status message.', { error: error instanceof Error ? error.message : String(error) });
      return false;
    }
  }
}
