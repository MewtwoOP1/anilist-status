...
export const commandData = [
  ...
  new SlashCommandBuilder().setName('test').setDescription('Check AniList right now and show whether there is a current outage.').setDefaultMemberPermissions(admin),
...
export async function handleCommand(interaction: ChatInputCommandInteraction, db: JsonDatabase, monitor: Monitor, alerts: AlertService, logger: Logger): Promise<void> {
  try {
    ...
    if (command === 'check') {
      ...
      const channelTestSent = await alerts.sendTest(result);
    ...
    if (command === 'test') {
      ...
      const result = await healthCheck(config.anilistUrl, config.requestTimeoutMs);
      const channelTestSent = await alerts.sendTest(result);
      const outageDetected =
        Boolean(state.outageStartedAt) ||
        result.status === 'DOWN' ||
      ...
      const description =
        result.status === 'HEALTHY'
          ...
          : result.status === 'RATE_LIMITED'
            ? 'AniList API is currently rate limited.'
            : result.status === 'DOWN'
      ...
      const e = embed(`${statusIcon(result.status)} AniList API Test`, description).addFields(
        { name: 'Status', value: `**${result.status}**`, inline: true },
        { name: 'HTTP', value: result.httpStatus == null ? 'N/A' : `**${result.httpStatus}**`, inline: true },
        { name: 'Response Time', value: `**${formatResponseTime(result.responseTimeMs)}**`, inline: true },
        { name: 'Current Outage', value: outageDetected ? '🔴 **YES**' : '🟢 **NO**', inline: true },
        { name: 'Checked', value: discordTimestamp(result.checkedAt), inline: true }
      );
      ...
      if (state.outageStartedAt) {
        e.addFields({
          ...
          value: `**${formatDuration(Date.now() - state.outageStartedAt)}**`,
          inline: true
        });
      }
      ...
      e.addFields({
        name: 'Alert Channel Message',
        value: channelTestSent ? '🟢 **SENT**' : '🔴 **NOT SENT**',
        inline: true
      });
    ...
    if (command === 'set-status-channel') {
      ...
      await alerts.sendStatusMessage();
      await interaction.reply({
        embeds: [
          embed(
            '✅ Status Channel Updated',
            `The status message is configured in <#${channel.id}>. It will be updated instead of creating a new message on every check.`
          )
        ],
